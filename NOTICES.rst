...
5.  ``src/dendropy/dataio/_beast2_v2_7_8_lark_standalone.py`` is generated by
    ...
    "Standalone Mode" code generator, from the grammar documented above
    ``parse_comment_metadata_beast2_v2_7_8`` in
    ``src/dendropy/dataio/nexusprocessing.py``, which also gives the
    regeneration command. Lark is not a DendroPy dependency: the generated
    module is self-contained, and only regenerating it requires Lark.
    ...
    per the notice embedded in Lark's generator, under the
    `Mozilla Public License, v. 2.0 <https://mozilla.org/MPL/2.0/>`_.
